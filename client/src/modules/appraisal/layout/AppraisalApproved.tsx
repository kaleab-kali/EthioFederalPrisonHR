...
const AppraisalApproved = () => {
  ...
  const pdfFunction = () => {
        ...
        <Page size="A4" style={styles.page}>
          <View style={styles.table}>
            <View style={styles.tableRow}>
              ...
              <View style={styles.tableCol}>
                <Text style={styles.tableCell}>Current Level</Text>
              </View>
              <View style={styles.tableCol}>
                <Text style={styles.tableCell}>Next Level</Text>
              </View>
              <View style={styles.tableCol}>
              ...
              <View style={styles.tableCol}>
                <Text style={styles.tableCell}>Promotion Date</Text>
              </View>
            </View>
            {data.map((row, index) => (
              <View key={index} style={styles.tableRow}>
                ...
                <View style={styles.tableCol}>
                  <Text style={styles.tableCell}>{row.currentLevel}</Text>
                </View>
                <View style={styles.tableCol}>
                  <Text style={styles.tableCell}>{row.nextLevel}</Text>
                </View>
                <View style={styles.tableCol}>
                ...
                <View style={styles.tableCol}>
                  <Text style={styles.tableCell}>
                    {row.promotionDate?.toLocaleDateString() || "N/A"}
                  </Text>
                </View>
              </View>
            ))}
    ...
    handleDownloadPDF(MyDocument, 'appraisal_approved.pdf');
  };

  const excelFunction = () => {
    const worksheet = XLSX.utils.json_to_sheet(data);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Appraisal Approved");
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(blob, 'appraisal_approved.xlsx');
  };
  ...
  const csvData = data.map((row) => ({
    ...
    status: row.status,
    promotionDate: row.promotionDate?.toLocaleDateString() || "N/A",
  }));
  ...
  return (
    <>
      ...
      <div className="flex justify-end gap-2 mb-3">
        ...
        <CSVLink data={csvData} filename={"appraisal_approved.csv"} className="bg-green-300 rounded-md px-4 text-center text-md no-underline">
          CSV
        </CSVLink>
